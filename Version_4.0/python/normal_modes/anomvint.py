...
def arguments(args=None):
    ...
    tosum3 = v3.sel(lev=slice(pfa, pfb)) 

    #print(" cnt = ",cnt)
    #if (cnt > 0): 
    et1=getattr(tosum1,f"etc{ca}{cb}").sum(dim="lev")
    et2=getattr(tosum2,f"etc{ca}{cb}").sum(dim="lev")
    etca=(et1.isel(time=0).values-et2.isel(time=0).values)/1000.0

    etca_da = xr.DataArray(
        etca,
        dims=('lat', 'lon'),
